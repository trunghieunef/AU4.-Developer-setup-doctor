...
def _ai_summary(report: Report) -> str:
    if report.ai_status is None:
        return "AI: disabled (rule-based remediation only)"
    ai = report.ai_status
    detail = f"AI: {ai.status} ({ai.provider}/{ai.model})"
    if ai.suggestions_added:
        detail += f"; {ai.suggestions_added} suggestion(s) added"
    return f"{detail}\n{ai.message}" if ai.message else detail
...
def print_report(report: Report, target: Console | None = None) -> None:
    ...
    target.print(Panel(
        Group(
            ...
            Text(_ai_summary(report), style="dim"),
        ),
    ...
    if failed:
        ...
        for check in failed:
            ...
            for index, step in enumerate(check.remediation, 1):
                ...
                source = "[AI]" if step.source == "ai" else "[RULE]"
                style = "cyan" if step.source == "ai" else "white"
                fixes.append(Text(f"  {index}. {source} {step.step}{command}", style=style))
        target.print(Panel(
